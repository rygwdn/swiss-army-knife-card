 ...
 * @returns {number}
 */
const findFirstValuedIndex$1 = (stops, startIndex) => {
 ...
 */
const interpolateStops$1 = (stops) => {
  ...
  return stops.map((stop, stopIndex) => {
    ...
    if (rightValuedIndex == null) {
      rightValuedIndex = findFirstValuedIndex$1(stops, stopIndex);
...
const computeThresholds$1 = (stops, type) => {
  const valuedStops = interpolateStops$1(stops);
  try {
...
class SparklineGraphTool extends BaseTool {
  constructor(argToolset, argConfig, argPos) {
    ...
    this.config.sparkline.colorstops.colors = computeThresholds$1(
...
class ProgressPathTool extends BaseTool {
  constructor(argToolset, argConfig, argPos) {
  ...
  */
  renderMarkerSvg() {
    ...
    this.intColor(this._stateValue, 0);
  ...
  renderTrackColorstops() {
    if ((this.config.progpath.show.track === TRACK_TYPE.COLORSTOPS) && (this.config.progpath.colorstops)) {
      ...
      Utils.calculateSvgDimension(this.config.progpath.track.width);
      let maskBegin;
